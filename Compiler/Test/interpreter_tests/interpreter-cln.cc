...
enum OutputCode { READ, WRITE, LOAD, STORE, COPY, ADD, SUB, SHR, SHL, INC, DEC, RESET, JUMP, JZERO, JODD, HALT, ERROR };

int main(int argc, char* argv[])
{
    std::vector< tuple<OutputCode,int,int> > program;
    map<cl_I,cl_I> pam;
    ...
    if( argc!=2 )
    {
	std::cout << "Sposób użycia programu: interpreter kod" << endl;
	return -1;
    }

    std::cout << "Czytanie programu." << endl;
    ifstream plik( argv[1] );
    if( !plik )
    {
	std::cout << "Błąd: Nie można otworzyć pliku " << argv[1] << endl;
	return -1;
    }
    while( !plik.eof() )
    {
	...
	if( com=="RESET" ) { i1 = RESET; plik >> i2; }
        ...
        if(i2>9) { std::cout << "Błąd: zły adress w instrukcji w linii " << k << endl; return -1; }
        if(i3>9) { std::cout << "Błąd: zły adress w instrukcji w linii " << k << endl; return -1; }
	...
	if(i1==ERROR) { std::cout << "Błąd: Nieznana instrukcja w linii " << k << "." << endl; return -1; }
        if(i2<0) { std::cout << "Błąd: Zły adress w instrukcji w linii " << k << endl; return -1; }
        if(i3<0) { std::cout << "Błąd: Zły adress w instrukcji w linii " << k << endl; return -1; }
    ...
    plik.close();
    std::cout << "Skończono czytanie programu (linii: " << program.size() << ")." << endl;

    std::cout << "Uruchamianie programu." << endl;
    lr = 0;
    srand(time(NULL));
	...
	{
	    case READ:	std::cout << "? "; cin >> r[get<1>(program[lr])]; i+=100; lr++; break;
	    case WRITE:	std::cout << "> " << r[get<1>(program[lr])] << endl; i+=100; lr++; break;
	...
	{
	    std::cout << "Błąd: Wywołanie nieistniejącej instrukcji nr " << lr << "." << endl;
	    return -1;
	}
    }
    std::cout << "Skończono program (czas: " << i << ")." << endl;
