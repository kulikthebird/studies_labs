...
class EntropyTga : public Entropy
{
public:
    EntropyTga(const char* p_path) : Entropy(p_path) {}
    std::vector<double> count();

private:
    void countColors();

    OneCharFrequency m_freqR = std::vector(256, 0);
    OneCharFrequency m_freqG = std::vector(256, 0);
    OneCharFrequency m_freqB = std::vector(256, 0);
    OneCharFrequency m_oneCharFreq = std::vector(256, 0);
};

std::vector<double> EntropyTga::count()
{
    countColors();
    return {countEntropy(m_oneCharFreq, m_fileSize),
                countEntropy(m_freqR, m_fileSize),
                countEntropy(m_freqG, m_fileSize),
                countEntropy(m_freqB, m_fileSize)};
}

void EntropyTga::countColors()
{
    unsigned char l_byte;
    while (m_file.read((char*)&byte, sizeof(char)))
    {
        ...
        if (m_fileSize % 3 == 0)
            m_freqB[l_byte]++;
        else if (m_fileSize % 3 == 1)
            m_freqG[l_byte]++;
        else if (m_fileSize % 3 == 2)
            m_freqR[l_byte]++;
    }
}
